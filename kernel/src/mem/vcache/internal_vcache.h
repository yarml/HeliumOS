...
unused static uint16_t pde_free_pages(mem_pde_ref *pde) {
  uint16_t meta = mem_vpstruct_ptr_meta(pde);
  return meta & 0x3ff;
}

unused static uint16_t pde_lazy_pages(mem_pde_ref *pde) {
  return mem_vpstruct_ptr_meta(pde) >> 10 & 0x3f;
}

unused static void pde_set_free(mem_pde_ref *pde, uint16_t free) {
  uint16_t meta = mem_vpstruct_ptr_meta(pde);
  mem_vpstruct_ptr_set_meta(pde, (meta & 0xfc00) | (free & 0x3ff));
}

unused static void pde_set_lazy(mem_pde_ref *pde, uint16_t lazy) {
  uint16_t meta = mem_vpstruct_ptr_meta(pde);
