...
void vcache_init() {
  ...
  target_pdpt =
      (mem_pdpte_ref *)SS_PADR(target_pml4e) + ENTRY_IDX(2, VCACHE_PTR);

  // Chances are, this condition is definitly always true
  if (!target_pdpt->present) s_alloc_substruct(target_pdpt);

  i_vcache_pde = (mem_pde_ref *)SS_PADR(target_pdpt) + ENTRY_IDX(1, VCACHE_PTR);
  ...
  for (size_t i = 0; i < PDE_COUNT; ++i) {
    ...
    if (vu.error) error_general("VCache init", "Could not map vcache PT");
