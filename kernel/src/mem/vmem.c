...
errno_t mem_vmap(void *vadr, void *padr, size_t size, int flags) {
  ...
  if (vmap_unit[0].error) {
    if (!vmap_unit[1].error) vcache_umap(vmap_unit[1], 0);
    prtrace_end("mem_vmap", "ERR_MEM_NO_VC_SPACE", 0);
    return ERR_MEM_NO_VC_SPACE;
  }

  if (vmap_unit[1].error) {
    if (!vmap_unit[0].error) vcache_umap(vmap_unit[0], 0);
    prtrace_end("mem_vmap", "ERR_MEM_NO_VC_SPACE", 0);
    return ERR_MEM_NO_VC_SPACE;
  }
  ...
  while (mapped < size) {
    ...
    while (order > target_order) {
      ...
      if (!target_entry->present) {
        ...
        // we always set it as a user page ref, because it can
        // contain sub pages that are either the system's ot the
        // user's If I was sure the kernel had 0 vulnerability,
        // I could set it as as user page even if it is above
        // KVMSPACE, and instead unset the user flag in the
        // individual page_map. But there could be
        // vulnerabilities in some other code, so a doubl
        // protection can't hurt
        if (vadr < KVMSPACE) target_entry->user = 1;
      ...
      // So here, we are sure that the current VStruct entry is
      // present We map it's substructure into the coresponding
      // VCache If it is already mapped, vcache_remap will handle it
      // and skip re-remapping it
    ...
    if (use_invlpg) as_invlpg((uint64_t)vadr);
...
errno_t mem_vumap(void *vadr, size_t size) {
  // Trashy temporary implementation, that only marks the page as not
  // present This trashy temporary implementation does not bother freeing
  // the physical space used by virtual memory structures when it's
  // currently unneeded Me lazy rn, I just want it to work
...
static void recursive_find_vseg(size_t req, vcache_unit *cache,
  ...
  } else  // PTE
  {
    ...
    if (!cpte->present) {
      if (!*seg_size) *seg_ptr = PTR_MAKE_CANONICAL(indices[0] * MEM_PS);
      *seg_size += MEM_PS;
      if (*seg_size >= req) return;
...
mem_vseg mem_find_vsegment(size_t size, void *heap_start, size_t heap_size) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    if (cache[i].error) {
      for (size_t j = 0; j < i; ++j) vcache_umap(cache[j], 0);
  ...
  if (seg_size < size) return (mem_vseg){0, 0, MEM_VSEG_ERROR_NOT_FOUND};
