...
mem_pallocation mem_ppalloc(void *pheader, size_t size, size_t alignment,
                            bool cont, void *below) {
  prtrace_begin("mem_ppalloc",
        ...
        && (!below || h->padr < below)) /* lowest address low enough */
    {
      ...
      if (bitmap[first_found_idx] != UINT64_MAX) {
        ...
        while (pg_idx * MEM_PS < h->size          /* still in segment */
               && (!below ||
                   h->padr + pg_idx * MEM_PS < below)    /* respecting below */
                                                     ...
                                                     is enough */
          {
            ...
            prtrace_end(
                ...
                alloc.header_off, alloc.padr, alloc.size, fpg_idx, lpg_idx);
            return alloc;
          }
          ...
          if (ffs == -1) {
            first_found_idx =
                bitmap_size / 8 -
                as_nscasq((uint64_t)(bitmap + pg_idx / 64 + 1), UINT64_MAX,
...
void mem_ppfree(void *pheader, mem_pallocation alloc) {
  prtrace_begin("mem_ppfree",
                "pheader=%p, alloc={header_off=%p, padr=%p, size=%lu}", pheader,
