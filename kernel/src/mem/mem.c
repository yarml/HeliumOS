...
size_t i_order_ps[ORDER_COUNT] = {[0] = (size_t)MEM_PS,
                                  [1] = (size_t)512 * MEM_PS,
...
void mem_init() {
  ...
  */
  ...
  for (size_t i = mmap_len; i != 0; --i) {
    printd("mregion(type=%s, padr=%p, size= %lz)\n",
  ...
  for (size_t i = 0; i < i_mmap_usable_len; ++i) {
    ...
    *last = UINT64_MAX << (h->size / MEM_PS) % 64;
    ...
    printd("header(header_adr=%016p,padr=%016p,size=%05lu)\n", h, h->padr,
           h->size / MEM_PS);
  }
  printd("pmm header(adr=%016p,offset=%05lu, size=%05lu)\n", i_pmm_header,
         pmm_header_off, pmm_header_total_size);
